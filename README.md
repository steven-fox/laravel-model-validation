...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/laravel-model-validation.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/laravel-model-validation)
...
```bash
php artisan vendor:publish --tag="laravel-model-validation-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-model-validation-config"
```
...
```bash
php artisan vendor:publish --tag="laravel-model-validation-views"
```
...
echo $laravelModelValidation->echoPhrase('Hello, StevenFox!');
```
